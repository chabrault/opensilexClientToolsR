...
MultiLineString <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      MultiLineStringObject <- list()
      if (!is.null(self$`type`)) {
        MultiLineStringObject[['type']] <- self$`type`
      }
      if (!is.null(self$`bbox`)) {
        MultiLineStringObject[['bbox']] <- self$`bbox`
      }
      if (!is.null(self$`coordinates`)) {
        MultiLineStringObject[['coordinates']] <- lapply(self$`coordinates`, function(x) x$toJSON())
      }

      MultiLineStringObject
    },
    fromJSON = function(MultiLineStringJson) {
      MultiLineStringObject <- jsonlite::fromJSON(MultiLineStringJson)
      if (!is.null(MultiLineStringObject$`type`)) {
        self$`type` <- MultiLineStringObject$`type`
      }
      if (!is.null(MultiLineStringObject$`bbox`)) {
        self$`bbox` <- MultiLineStringObject$`bbox`
      }
      if (!is.null(MultiLineStringObject$`coordinates`)) {
        self$`coordinates` <- lapply(MultiLineStringObject$`coordinates`, function(x) {
    ...
    fromJSONObject = function(MultiLineStringObject) {
      if (!is.null(MultiLineStringObject$`type`)) {
        self$`type` <- MultiLineStringObject$`type`
      }
      if (!is.null(MultiLineStringObject$`bbox`)) {
    ...
    fromJSONString = function(MultiLineStringJson) {
      ...
      self$`type` <- MultiLineStringObject$`type`
      self$`bbox` <- MultiLineStringObject$`bbox`
      self$`coordinates` <- lapply(MultiLineStringObject$`coordinates`, function(x) LngLatAlt$new()$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE)))
    }
  )
)
